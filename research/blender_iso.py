"""Render the four shell variants (normal/radial x width/delete) in Blender.
...
def main():
    for stem in ("miffy", "zichaoxiong"):
        data = np.load(os.path.join(BLEND, f"iso_{stem}.npz"))
        bv = data["body_v"]; bt = data["body_t"]
        combos = []
        for dname in ("normal", "radial"):
            for hname in ("width", "delete"):
                combos.append((f"{dname}_{hname}",
                               data[f"{dname}_{hname}_v"], data[f"{dname}_{hname}_t"]))

        for angle in ANGLES:
            for tag, sv, st in combos:
                clean()
                ...
                out = os.path.join(OUT, f"iso_{stem}_{tag}_{angle:03d}.png")
