"""Render the unmasked hull from below and behind, where the masks were claimed to help.

The build's comments say the downward-facing mask exists because Miffy's skirt underside
drew a sawtooth fringe across her legs. That is a checkable claim, so this renders the
current (masked) and unmasked shells from a low camera angle and from the back, and
marks any ink that lies well inside the body silhouette in red.

Run:
    blender.exe --background --factory-startup --python research/blender_below.py
"""
import math
import os
...
RES = 900
VIEWS = (("below", -35.0), ("back", 180.0), ("quarter", 215.0))
    ...
    for stem in ("miffy", "zichaoxiong"):
        for tag in TAGS:
            ...
            for view, angle in VIEWS:
                ...
                elev = math.radians(-35.0 if view == "below" else 12.0)
                ...
                cam.location = centre + Vector((
                    math.sin(a) * math.cos(elev) * dist,
                    -math.cos(a) * math.cos(elev) * dist,
                    math.sin(elev) * dist))
                ...
                path = os.path.join(OUT, f"view_{stem}_{tag}_{view}.png")
